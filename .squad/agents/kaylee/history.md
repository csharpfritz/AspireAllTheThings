...
- **2025-07-25 (Part 5 Docs):** Updated README Part 5 section to reflect parameter prompt changes — `Discord:WebhookUrl` user secret replaced by `Parameters:discordWebhookUrl` and `Parameters:discordChannel` via `AddParameter()`. Documented `WithDescription(enableMarkdown: true)` for rich Markdown prompt text. Updated code samples, key concepts table, and live demo flow. Created `part5-discord-demo-script.md` following the Part 6 demo script format. Build verified clean.
